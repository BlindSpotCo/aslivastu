...
## Second dimension pass (2026-08-06, same session, later)

Gurshaan asked for "all 8 dimensions or at least 7" for the 5 already-scored localities, plus
a list of areas added. Ran real research on infrastructure, power (PSPCL), water, roads,
sewerage, and a second look at air, for the same 5 localities (Sarabha Nagar, Dugri, Model
Town, Majitha Road, Rani Ka Bagh). **Only water cleared the bar for a numeric score
addition.** Full reasoning is embedded as comments directly above `master_records()` in
`pipeline/scrapers/punjab_data.py`; short version per dimension:

- **Infrastructure** — real facts exist for 2 of the 5 (confirmed: no metro in either city;
  NH-5 confirmed for Sarabha Nagar; a named Smart City project each for Sarabha Nagar and
  Hall Bazaar), but "not found" for the rest — treating "not found" as "not present" would
  assert a negative that wasn't actually confirmed. Skipped as a score.
- **Power (PSPCL)** — no locality-level outage data is published anywhere; the only number
  that exists (Ludhiana Main Town circle SAIFI/SAIDI) is from 2014-15, 11+ years stale and
  circle-wide, not locality-wide. Skipped as a score. Real qualitative signal exists (Majitha
  Road named "worst hit," roughly hourly cuts, Tribune Jun 2024) — used as an inspection note
  instead of a fabricated number.
- **Roads / Sewerage** — rich, real, dated incident reports exist (a recarpeted road peeling
  within 12 hours; 2+ years of chronic sewage in Model Town) but no official numeric rating
  is published anywhere. Converting news coverage into a 1-5 number would mean inventing the
  rating. Skipped as scores; used as inspection notes instead.
- **Air** — still blocked, now with a confirmed reason: this environment's outbound network
  cannot reach `*.gov.in`/`*.nic.in`/`data.gov.in` (403 at the CONNECT-tunnel level), which
  blocks CPCB's live AQI dashboard and the data.gov.in AQI API. Not a "didn't find it"
  problem — a documented environment constraint.
- **Water — added.** AIIB Environmental & Social Impact Assessment reports for the PMSIP
  bulk-water-supply projects quote real MC/DWSS-sourced **city-wide** (not locality-specific)
  daily supply hours: Ludhiana ~7 hrs/day (Aug 2024 ESIA), Amritsar ~12 hrs/day across 3
  blocks (May 2024 ESIA). Applied only to the 5 already-scored localities, which already have
  a genuinely locality-specific dimension (schools) to pair it with — deliberately NOT used
  to newly score Hall Bazaar, since a score built from 100% city-wide/0% locality-specific
  data would misrepresent itself even with `dimensions_scored:1/8` shown.

Also added: real, dated, sourced qualitative "inspection notes" (waterlogging, road-quality
incidents, power-cut reports) per locality — NOT part of the score, surfaced as extra
good/bad bullets on the report page via `notes_good`/`notes_bad`, merged into `highlights()`
in `pages/report/[pin].js` after the score-derived bullets.

**Recomputed scores (via the real `compute_nqi()`, not hand math):**

| Locality | Slug | Schools | Water | NQI | Grade | Was (v1) |
|---|---|---|---|---|---|---|
| Dugri | `ldh-dugri` | 90 | 54 | 74 | B+ | 90 / A+ |
| Majitha Road | `asr-majitha-road` | 75 | 63 | 70 | B+ | 75 / B+ |
| Sarabha Nagar | `ldh-sarabha-nagar` | 60 | 54 | 57 | C+ | 60 / B |
| Model Town | `ldh-model-town` | 60 | 54 | 57 | C+ | 60 / B |
| Rani Ka Bagh | `asr-rani-ka-bagh` | 20 | 63 | 39 | D | 20 / D |
| Hall Bazaar | `asr-hall-bazaar` | — | — | — | — | unscored (stays unscored) |

`dimensions_scored` moves from 1/8 to 2/8 for all five. `web/public/master_by_pin.json` and
`web/public/nqi_scores.json` were regenerated by running the real pipeline functions
(`_merge_schools`, `compute_nqi`, `add_crime_percentiles`, `add_price_context`,
`add_punjab_notes`) against the updated `scrapers/punjab_data.py`, confirmed to match this
table exactly, then written back — same not-fabricated verification standard as the first
batch. `scoring.py` gained one new function, `add_punjab_notes()`, called from `run()` right
after `add_price_context()`.
